...
describe("InisSandbox owned lifecycle", () => {
  ...
  it("forwards connections (and other session-create options) straight through to client.sessions.create()", async () => {
    const fake = new FakeSession("sess-owned-connections");
    ...
    // workspaceRoot is stripped before reaching sessions.create() -- it's
    // this adapter's own option, not a real CreateSessionOptions field.
    expect(createOpts).toMatchObject({ egress: { mode: "deny" }, connections });
    expect(createOpts).not.toHaveProperty("workspaceRoot");
