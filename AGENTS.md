...
- **Native framework adapter** — a package that wraps the core SDK for a
  specific agent framework: `inis-pydantic-ai` (Pydantic AI),
  `@inis-run/ai-sdk` (Vercel AI SDK), `langchain-inis` (LangChain Deep
  Agents), `inis-openai-agents` / `@inis-run/openai-agents` (OpenAI Agents
  SDK), `inis-strands-agents` / `@inis-run/strands-agents` (Strands Agents),
  `@inis-run/mastra` (Mastra Workspaces), `inis-google-adk` (Google ADK),
  and `inis-agent-framework` (Microsoft Agent Framework). All are currently
  **beta** — check
