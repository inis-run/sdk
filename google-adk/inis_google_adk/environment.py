...
4. **Real churn, not hypothetical.** `BaseEnvironment`/`EnvironmentToolset`'s
   ...
   instruction.
5. `ExecutionResult` (the dataclass `execute()` returns) has no `truncated`
   field -- just `exit_code`/`stdout`/`stderr`/`timed_out`. When inis.run's
   own per-stream capture limit truncates a command's output before it ever
   reaches this adapter, this module appends a visible marker to `stderr`
   instead of silently dropping that fact -- the same choice
   `inis_openai_agents.sandbox` makes for the identical dataclass-shape gap,
   including the random-nonce reasoning (a guest with full control of its
   own output could otherwise read this marker's exact wording out of this
   open-source file and print a fake one).
...
class InisEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        *,
        ...
        `egress_default=None` for the account default instead."""
    ...
    async def execute(
        ...
        *,
        ...
        if result.truncated:
            # ExecutionResult has no `truncated` field -- see module
            # docstring point 5 for why this is a visible stderr marker with
            # a fresh per-call nonce rather than a silently dropped fact or a
            # static (guest-spoofable) string.
