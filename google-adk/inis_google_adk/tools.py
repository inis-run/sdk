...
def inis_adk_tools(session: AsyncSession) -> list[FunctionTool]:
    ...
    async def execute(command: str) -> str:
        ...
        if result.truncated:
            # `execute()` returns one plain string -- the model-visible tool
            # result itself, same as `InisEnvironment.execute()`'s
            # `ExecutionResult` is not: there is no separate trusted field
            # this function could move the signal to (unlike
            # `InisEnvironment.execute()`/`InisSandboxSession.exec()`, which
            # now return it as a real `truncated` attribute).
            # A per-call nonce here used to imply this marker was
            # cryptographically hard to spoof; it was not, since nothing
            # anywhere checked it. Drop the nonce and treat this exactly
            # like the equally-unforgeable `[inis: command timed out]` /
            # `[inis: exited N]` annotations two lines up: an honest,
            # undecorated note to whatever reads this string (model
            # included), not a security boundary.
            marker = "[inis: output truncated by the sandbox's own capture limit]"
