...
class InisSandbox(PosixShellSandbox):
    ...
    async def create(
        ...
        **session_kwargs: Any,
        ...
        see the core `inis` SDK for the full set; `connections` binds this
        session to one or more external API origins with a credential the
        platform injects only at request time). `client` defaults to a fresh
