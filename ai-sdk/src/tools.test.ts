...
describe("createOwnedInisTools", () => {
  ...
  it("forwards connections through to the underlying session create call", async () => {
    const createSpy = vi
      .spyOn(Session, "create")
      .mockResolvedValue(
        Session.attach({ baseUrl: "https://api.inis.run", token: "t" } as Client, "ses_owned_conn"),
      );
    ...
    await createOwnedInisTools({ apiKey: "tok_test", connections });

    expect(createSpy).toHaveBeenCalledTimes(1);
    expect(createSpy.mock.calls[0]?.[1]).toMatchObject({ connections });
  });
});
