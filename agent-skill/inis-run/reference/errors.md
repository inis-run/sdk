...
| `session_ended` | | 410 | No | Ended past recall (idle timeout, max lifetime, or expired-paused-TTL) — start a new session. When the cause is a session past its own `max_lifetime_ms`, GET reports this ahead of time via `state: "lifetime_expired"` instead of only surfacing it once a resume attempt fails. |
| `session_node_lost` | | 410 | No | Home node declared permanently dead — recreate the session from its template, or restore from your last checkpoint; data since then is gone. GET still works and DELETE still destroys the session; every other operation returns this. |
