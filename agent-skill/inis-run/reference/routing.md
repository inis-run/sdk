...
- **PTY / interactive terminal** is wired into OpenAI Agents (both
  languages) — `exec_command(tty=True)`/`write_stdin` in Python,
  `exec({ tty: true })`/`writeStdin` in JS. Not available in
  `@inis-run/strands-agents`/`inis-strands-agents`: `strands.sandbox`'s
  `Sandbox`/`PosixShellSandbox` contract has no PTY concept to hook into at
  all, so that one isn't an adapter gap to close.
