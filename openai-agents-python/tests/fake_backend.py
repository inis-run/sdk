...
class FakeLocalPty:
    """Local-subprocess double for `inis.AsyncPTY`, returned by
    `LocalProcessInisBackend.pty()`. Runs the whole command to completion up
    front (real subprocess, same as `LocalProcessInisBackend._run`) and
    replays its combined output as one chunk followed by PTY_EXIT -- enough
    to exercise `InisSandboxSession.pty_exec_start`'s wiring for a
    non-interactive command (e.g. the `exec_command` tool's default path,
    which now goes through PTY first since `supports_pty()` is true). Real
    interactive round-tripping (`pty_write_stdin` against a still-running
    session) is covered separately in `test_sandbox.py::TestPty` against a
    hand-built fake that doesn't need a real process at all.
    """
    ...
    async def write(self, data: bytes | str) -> None:
        raise NotImplementedError(
            "FakeLocalPty is write-once/read-only (the command already ran to "
            "completion); use test_sandbox.py::TestPty's FakePTY for stdin round-trips"
        )
...
class LocalProcessInisBackend:
    ...
    async def pty(self, *, command: list[str] | None = None, **_ignored) -> FakeLocalPty:
        """Stands in for `AsyncSession.pty()` -- a real WebSocket method the
        `httpx.MockTransport` above can't fake, since it only intercepts
        HTTP. `attach_fake_session` monkeypatches this directly onto the
        `AsyncSession` in place of the real PTY client."""
        cmd = command or ["sh", "-l"]
        if cmd[:2] in (["sh", "-lc"], ["bash", "-lc"]):
            self.exec_log.append(cmd)
        result = await self._run(cmd, timeout_ms=None)
        combined = (result["stdout"] + result["stderr"]).encode("utf-8", errors="replace")
        return FakeLocalPty(combined, result["exit_code"])
...
def attach_fake_session(session_id: str, *, workspace_root: str = "/workspace"):
    ...
    session.pty = backend.pty  # see LocalProcessInisBackend.pty's docstring
