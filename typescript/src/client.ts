...
// retryable code: session_ended (410, definitively over), session_node_lost
// (410, the session's home node is permanently gone, not merely
// unreachable), archive_unavailable
...
const RETRYABLE_CODES = new Set([
  ...
  "bad_gateway",
  // A concurrent duplicate request for the same Idempotency-Key is still
  // being processed — retry and it resolves to the same session
  // once the other request finishes.
  ...
  "rate_limit_exceeded",
  "service_unavailable",
  // Account- and key-scoped concurrency caps -- same retryable-429 class as
  // rate_limited/concurrency_limit_exceeded above, kept as their own codes
  // so a caller can tell which ceiling it hit. Both are emitted with a
  // Retry-After header.
...
/** Sandbox tier: small (1 vCPU/1 GB), medium (2/4 GB), large (4/8 GB), xlarge (8/16 GB). */
export type SessionSize = "small" | "medium" | "large" | "xlarge";

/** Lifecycle states a session can be in. "lifetime_expired" is a
 * read-time-only presentation of a paused session whose own maxLifetimeMs
 * deadline has passed -- it's still just a paused session underneath (not
 * destroyed), but resume() will always reject (410): check this before
 * calling resume() on a long-paused session rather than discovering it from
 * a failed resume. See SessionInfo.statusReason. */
...
/**
 * Which signal the idle timer trusts before pausing a session. "ops"
 * (default) is today's op/PTY/keep_alive-based idle tracking; "activity"
 * defers the pause while the guest shows CPU or network activity above
 * threshold — for a session doing real work with no API ops.
 */
export type IdleMode = "ops" | "activity";
...
/** How a Connection authenticates to its bound origin: "bearer" injects
 * `Authorization: Bearer <secret>`; "header" injects the secret under a
 * caller-chosen header name. */
export interface ConnectionAuth {
  type: "bearer" | "header";
  /** The upstream credential's plaintext value. Sent once on create/rotate;
   * never echoed back in any response. */
  secret: string;
  /** Required (and validated as a safe, non-reserved header) for "header"
   * type; must be omitted, or exactly "Authorization", for "bearer". */
  headerName?: string;
}

/** Deny-by-default request allowlist for a Connection: an empty list
 * authorizes nothing. */
export interface ConnectionAllow {
  /** HTTP methods this Connection accepts, e.g. ["GET", "POST"]. */
  methods: string[];
  /** Absolute path prefixes, e.g. ["/v1/customers"] or ["/v1/*"] for a
   * prefix match. */
  paths: string[];
}

/**
 * A session-scoped credential lease: binds one session to one external API
 * origin (`https://api.stripe.com`, say) and injects `authentication.secret`
 * into requests that clear `allow`, so code in the session can call that
 * origin with whatever SDK it already uses — no proxy to configure, no
 * rewritten URL.
 *
 * The credential is never placed in your source, image, or the session's
 * `env`/`secrets` — it's injected on the way out by the platform, not by
 * anything running in your code. That said, it is not hidden from the
 * *sandbox's own outbound traffic*: if the bound origin reflects request
 * headers back (a debug/echo endpoint, an error body that happens to
 * include what it received), code running in the session can read the
 * value out of that response. Prefer a scoped, short-lived upstream
 * credential (a Stripe restricted key, a GitHub fine-grained token, an AWS
 * STS credential) wherever the provider offers one, so a reflected value on
 * its own is worth little — see the Connections docs for the full limit.
 */
export interface ConnectionCreate {
  /** Label for this Connection, unique within the session. Lowercase
   * alphanumerics, ".", "_", "-". */
  name: string;
  /** Exact `https://` origin — hostname only, no path, query, or port
   * other than 443. No wildcards; one Connection per origin per session. */
  origin: string;
  authentication: ConnectionAuth;
  allow: ConnectionAllow;
  /**
   * Lease lifetime in seconds. Defaults to the session's own
   * `maxLifetimeMs`, or 1 hour if the session has no lifetime cap. Capped
   * at 24h regardless, and rejected outright if it would outlive the
   * session — there's no way to mint a Connection that outlasts it.
   */
  ttlSeconds?: number;
}

/**
 * Body for `Session.rotateConnection()` — the same shape as
 * ConnectionCreate minus `name` (fixed by the URL this hits). `origin` must
 * match the existing Connection's origin; moving a Connection to a
 * different origin is rejected (revoke it and create a new session with
...
/** A Connection as reported on `SessionInfo`, redacted — name, origin,
 * state, header name, allow rules, and expiry, never the secret value.
 * "expired" is reported lazily: a lease past its TTL flips from "active" to
 * "expired" the next time the session is read, not the instant it lapses. */
...
export interface SessionInfo {
  ...
  /** Set when state=="failed" (a short, user-safe reason provisioning
   * didn't complete) or state=="lifetime_expired" (a short, user-safe
   * reason resume() will reject). Empty otherwise. */
  statusReason?: string;

  // Identity
  name?: string;
  labels?: Record<string, string>;
  /** Caller-owned index key set at create, if any. Not unique — multiple
   * sessions may share one. See CreateSessionOptions.externalId. */
  externalId?: string;
  ...
  /** Which signal the idle timer trusts before pausing: "ops" (default,
   * op/PTY/keep_alive-based) or "activity" (defers while the guest shows
   * CPU or network activity above threshold). */
  idleMode?: IdleMode;
  /** Per-session override of the activity-mode CPU threshold (percent of
   * one vCPU). 0/undefined means this session uses the fleet-wide default. */
  idleBusyCpuThresholdPct?: number;
  /** Per-session override of the activity-mode network threshold (combined
   * guest tx+rx bytes/sec). 0/undefined means this session uses the
   * fleet-wide default. */
  idleBusyNetworkBytesPerSec?: number;
  maxLifetimeMs?: number;
  onPtyDetach?: OnPtyDetach;
  pausedTtlMs?: number;
  noSudo?: boolean;
  /** Whether an inbound request to a preview URL may wake this session from
   * paused or cold storage. Always present: true is the default (matches
   * today's unconditional wake-on-HTTP behaviour), false means the session
   * opted out at create — see CreateSessionOptions.wakeOnHttp. */
  wakeOnHttp?: boolean;
  /** The host-served self API's per-session gate state (e.g. {events:
   * false}) — see /docs/self-api.md. A workload inside the VM reaches this
   * surface directly at http://169.254.169.254; there is no dedicated SDK
   * wrapper for it yet, so this is the raw response object. */
  selfApi?: Record<string, unknown>;
  ...
  /**
   * Connections bound to this session, redacted — name, origin, state,
   * header name, allow rules, and expiry, never the secret value. See
   * CreateSessionOptions.connections.
   */
...
export interface TemplateInfo {
  ...
  /** Set only on an alias entry: a documented bare name that is not
   * itself a published template family but currently resolves to one, e.g.
   * name "python" with aliasOf "python-3.13" and version "2026.07.10".
   * Omitted for a real (non-alias) template. */
  aliasOf?: string;
}
 ...
 * session or one-shot run mounts it. Created explicitly via
 * `volumes.create()` and referenced afterwards by its server-issued `id`
 * as `volumeId` on session-create/run — naming an unrecognized or another
 * tenant's id there throws InisError (404); lazy creation is retired.
 *
 * Deliberately node-agnostic: no node id or placement hint anywhere in
 * this shape, so where a volume's data physically lives can change
 * without that ever becoming a breaking change here. */
export interface VolumeInfo {
  id: string;
  sizeGb: number;
  createdAt: string;
  attached: boolean;
  /** Set only when attached is true. */
  sessionId?: string;
  /** When this volume's durable copy last finished being written to object
   * storage, sourced from the control-plane-blessed generation's own
   * archive manifest. Pushed on session *destroy*, not pause -- a paused
   * session still holds the volume attached, so this reflects the last
   * detach, not the last write.
   *
   * `undefined` for a volume that has never completed a push -- never a
   * zeroed or synthesized timestamp. A volume with no durable copy yet is
   * genuinely distinct from one saved long ago. */
  lastSavedAt?: string;
}
...
/** One bucket of `usage({ groupBy: "label:<key>" })`'s cost breakdown. */
export interface UsageLabelCost {
  label: string;
  costCents: number;
}

/** Aggregate compute/storage usage and spend for the org, see
 * `Client.usage()`. Every figure is read from the same
 * credit_transactions/usage_events ledger rows billing itself debited
 * from -- never re-derived from duration x rate. */
export interface UsageSummary {
  ...
  vmSeconds: number;
  vmMs: number;
  pausedStorageCents: number;
  volumeStorageCents: number;
  costToDateCents: number;
  pendingMillicents: number;
  computePendingMillicents: number;
  pausedPendingMillicents: number;
  volumePendingMillicents: number;
  balanceCents: number;
  /** Present only when `groupBy: "label:<key>"` was passed to `usage()`.
   * Sums to costToDateCents. */
  labels?: UsageLabelCost[];
}

/** One session's lifetime usage/cost, see `sessions.usage()`. No window --
 * cost-to-date means since the session started. */
...
export interface CreateSessionOptions {
  ...
  labels?: Record<string, string>;
  /**
   * Optional caller-owned index key (a user/workflow/thread id) to stamp
   * this session with, so it can be found later via
   * `client.sessions.list({ externalId })` without tracking sessionId
   * yourself. Not unique — multiple sessions may share one.
   */
  externalId?: string;
  /**
   * Optional retry-safe key. Calling Session.create/client.sessions.create
   * again with the same idempotencyKey returns the ORIGINAL session instead
   * of creating a second one or erroring on a name clash — use it when a
   * create's result may have been lost (e.g. a timeout) and you're not sure
   * whether the session was actually created. Sent as the `Idempotency-Key`
   * header, never in the request body. Reusing a key with different create
   * options is an error (409 conflict), not a silent replay of the first
   * call — use a new key for a genuinely different create.
   */
  idempotencyKey?: string;
  volumeId?: string;
  maxLifetimeMs?: number;
  idleTimeoutMs?: number;
  /** Which signal the idle timer trusts before pausing: "ops" (default) or
   * "activity". See IdleMode's doc. idleTimeoutMs is floored to 30s in
   * activity mode to prevent pause-storms. */
  idleMode?: IdleMode;
  egress?: EgressPolicy;
  template?: string;
  size?: SessionSize;
  noSudo?: boolean;
  /** Opt this session OUT of wake-on-HTTP. Omitted or true (the default)
   * keeps today's behaviour: any request to an exposed port wakes a paused
   * session automatically. false means a preview-URL request to a paused
   * or cold session returns 404 instead of waking it, with no charge.
   * Explicit resume/exec/other API calls are unaffected either way. */
  wakeOnHttp?: boolean;
  onPtyDetach?: OnPtyDetach;
  pausedTtlMs?: number;
  ...
  connectors?: string[];
  /**
   * Inline Connections to create with this session: session-scoped
   * credential leases that bind it to specific external API origins (see
   * `ConnectionCreate`, `SessionInfo.connections`, and
   * `Session.rotateConnection`/`revokeConnection` for the live-session
   * verbs). Up to 16 per session.
   *
   * The credential is injected by the platform and never placed in your
   * code, image, or the session's `env`/`secrets` — but code running in the
   * session can still read it back if the bound origin reflects request
   * headers. Prefer a scoped, short-lived upstream credential for anything
   * you bind this way; see the Connections docs for the full limit.
   */
  connections?: ConnectionCreate[];
  /** Enable the in-guest event channel (POST /v1/self/events → org webhooks
   * as guest.<type>). Off by default. */
  events?: boolean;
  /**
...
export interface ListSessionsOptions {
  ...
  cursor?: string;
  /**
   * Filter to sessions whose externalId exactly matches this value. Not
   * unique — may return more than one session.
   */
  externalId?: string;
}
...
function mapSessionInfo(data: Record<string, unknown>): SessionInfo {
  ...
  return {
    ...
    idleMode: data.idle_mode ? (String(data.idle_mode) as IdleMode) : undefined,
    idleBusyCpuThresholdPct:
      data.idle_busy_cpu_threshold_pct != null ? Number(data.idle_busy_cpu_threshold_pct) : undefined,
    idleBusyNetworkBytesPerSec:
      data.idle_busy_network_bytes_per_sec != null ? Number(data.idle_busy_network_bytes_per_sec) : undefined,
    ...
    wakeOnHttp: data.wake_on_http != null ? Boolean(data.wake_on_http) : undefined,
    selfApi: data.self_api ? (data.self_api as Record<string, unknown>) : undefined,
    ...
    connections: data.connections
      ? (data.connections as Record<string, unknown>[]).map(mapConnectionStatus)
      : undefined,
  };
}

function serializeConnectionAuth(auth: ConnectionAuth): Record<string, unknown> {
  const out: Record<string, unknown> = { type: auth.type, secret: auth.secret };
  if (auth.headerName) out.header_name = auth.headerName;
  return out;
}

function serializeConnectionCreate(connection: ConnectionCreate): Record<string, unknown> {
  const out: Record<string, unknown> = {
    name: connection.name,
    origin: connection.origin,
    authentication: serializeConnectionAuth(connection.authentication),
    allow: { methods: connection.allow.methods, paths: connection.allow.paths },
  };
  if (connection.ttlSeconds != null) out.ttl_seconds = connection.ttlSeconds;
  return out;
}

function serializeConnectionUpdate(connection: ConnectionUpdate): Record<string, unknown> {
  const out: Record<string, unknown> = {
    origin: connection.origin,
    authentication: serializeConnectionAuth(connection.authentication),
    allow: { methods: connection.allow.methods, paths: connection.allow.paths },
...
function mapConnectionStatus(data: Record<string, unknown>): ConnectionStatus {
  const allow = (data.allow as Record<string, unknown> | undefined) ?? {};
  return {
    name: String(data.name ?? ""),
    origin: String(data.origin ?? ""),
    state: String(data.state ?? "active") as "active" | "expired" | "revoked",
    headerName: data.header_name ? String(data.header_name) : undefined,
    allow: {
      methods: (allow.methods as string[] | undefined) ?? [],
      paths: (allow.paths as string[] | undefined) ?? [],
    },
...
function mapTemplate(data: Record<string, unknown>): TemplateInfo {
  return {
    ...
    aliasOf: data.alias_of ? String(data.alias_of) : undefined,
  };
}
...
function mapVolume(data: Record<string, unknown>): VolumeInfo {
  return {
    id: String(data.id ?? ""),
    sizeGb: typeof data.size_gb === "number" ? data.size_gb : 0,
    createdAt: String(data.created_at ?? ""),
    attached: Boolean(data.attached),
    sessionId: data.session_id ? String(data.session_id) : undefined,
    lastSavedAt: data.last_saved_at ? String(data.last_saved_at) : undefined,
  };
}
...
function mapUsageSummary(data: Record<string, unknown>): UsageSummary {
  const rawLabels = data.labels as Record<string, unknown>[] | undefined;
  return {
    periodStart: String(data.period_start ?? ""),
    since: String(data.since ?? ""),
    until: String(data.until ?? ""),
    executions: Number(data.executions ?? 0),
    vmSeconds: Number(data.vm_seconds ?? 0),
    vmMs: Number(data.vm_ms ?? 0),
    pausedStorageCents: Number(data.paused_storage_cents ?? 0),
    volumeStorageCents: Number(data.volume_storage_cents ?? 0),
    costToDateCents: Number(data.cost_to_date_cents ?? 0),
    pendingMillicents: Number(data.pending_millicents ?? 0),
    computePendingMillicents: Number(data.compute_pending_millicents ?? 0),
    pausedPendingMillicents: Number(data.paused_pending_millicents ?? 0),
    volumePendingMillicents: Number(data.volume_pending_millicents ?? 0),
    balanceCents: Number(data.balance_cents ?? 0),
    labels: rawLabels
      ? rawLabels.map((l) => ({ label: String(l.label ?? ""), costCents: Number(l.cost_cents ?? 0) }))
      : undefined,
  };
}

function mapSessionUsage(data: Record<string, unknown>): SessionUsage {
  return {
    sessionId: String(data.session_id ?? ""),
    executions: Number(data.executions ?? 0),
    vmSeconds: Number(data.vm_seconds ?? 0),
...
export class Session {
  ...
  static async create(client: Client, opts?: CreateSessionOptions): Promise<Session> {
    ...
    if (opts?.externalId) payload.external_id = opts.externalId;
    ...
    if (opts?.idleMode) payload.idle_mode = opts.idleMode;
    if (opts?.egress) {
    ...
    if (opts?.wakeOnHttp === false) payload.wake_on_http = false;
    ...
    if (opts?.connections) payload.connections = opts.connections.map(serializeConnectionCreate);
    if (opts?.events) payload.events = true;
    ...
    const data = await request<{
      ...
      client.baseUrl, client.token, "POST", "/v1/sessions", payload, client.timeoutMs, extraHeaders,
    );
  ...
  /** Fetch this session's lifetime usage/cost -- the same ledger-derived
   * figures Client.usage()'s org totals are built from, scoped to this
   * session (no window: cost-to-date means since the session started). */
  async usage(): Promise<SessionUsage> {
    const data = await request<Record<string, unknown>>(
      this._baseUrl, this._token, "GET",
      `/v1/sessions/${this.sessionId}/usage`, undefined, this._timeoutMs,
    );
    return mapSessionUsage(data);
  }
  ...
  /**
   * Rotate an existing Connection's credential, or replace its allow
   * rules/TTL, on a live or paused session.
   * PUT /v1/sessions/{id}/connections/{name}.
   *
   * The Connection named `name` must already exist (404 if it doesn't —
   * this does not create a new one), and `connection.origin` must match its
   * current origin: the API rejects moving a Connection to a different
   * origin (400) — revoke it and create a new session with the new origin
   * instead. Same validation as session create otherwise: TTL default/cap,
   * deny-by-default methods/paths.
   *
   * Not implemented on this client yet: adding a brand-new Connection to an
   * already-live session, and listing a session's Connections independently
   * of `Session.get()`'s redacted `connections` field — both are landing
   * separately. Use `connections` on `Session.create()` /
   * `Client.sessions.create()` for a session's initial set.
   */
  async rotateConnection(name: string, connection: ConnectionUpdate): Promise<ConnectionStatus> {
    const data = await request<Record<string, unknown>>(
      this._baseUrl, this._token, "PUT",
      `/v1/sessions/${this.sessionId}/connections/${encodeURIComponent(name)}`,
      serializeConnectionUpdate(connection), this._timeoutMs,
    );
    return mapConnectionStatus(data);
  }

  /**
   * Revoke a Connection immediately: its stored credential is deleted and a
   * tombstone marks the origin revoked for the rest of the session's life.
   * Fails closed — a later request to that origin gets a TLS-level failure,
   * never a silent fallback to an unauthenticated direct connection.
   * DELETE /v1/sessions/{id}/connections/{name}. 404 if no Connection by
   * that name exists on this session.
   */
  async revokeConnection(name: string): Promise<void> {
    await request<void>(
      this._baseUrl, this._token, "DELETE",
      `/v1/sessions/${this.sessionId}/connections/${encodeURIComponent(name)}`,
      undefined, this._timeoutMs,
    );
  }
...
export class Client {
  ...
  readonly sessions: {
    ...
    /** Fetch lifetime usage/cost for a session by ID without attaching to it. */
   ...
   * outlives whatever session or one-shot run mounts it. Lazy creation is
   * retired — a volume must be created here before its id can be passed as
   * `volumeId` on session-create/run; an unrecognized or cross-tenant id
   * there now throws InisError (404) instead of silently provisioning
   * one. */
  readonly volumes: {
    /** Create a new, empty persistent volume and return its server-issued
     * id. Omitted `sizeGb` applies the server default; a value over the
     * server's maximum throws InisError (400) rather than being silently
     * clamped down to it. Grow it later with `resize()` (grow-only). */
    create(opts?: { sizeGb?: number }): Promise<VolumeInfo>;
     ...
     * Throws InisError (404) for an unknown or another tenant's id,
     * indistinguishable from one that never existed. */
    get(volumeId: string): Promise<VolumeInfo>;
    /** Grow a volume's capacity. Grow-only — `sizeGb` must be strictly
     * greater than the volume's current size; throws InisError (400, code
     * "bad_request") for a same-size or smaller request rather than
     * treating it as a no-op or a shrink, and for a value over the
     * server's maximum. Throws InisError (409, code "conflict") while the
     * volume is attached to a live or paused session, or while an
     * attach/detach is in progress — detach it first. The larger size
     * takes effect the next time the volume is attached, not
     * immediately. */
    resize(volumeId: string, sizeGb: number): Promise<VolumeInfo>;
    /** Permanently delete a persistent volume (right-to-erasure/
     * offboarding). Throws InisError (409, code "conflict") while the
     * volume is attached to a live or paused session — destroy that
     * session first. Throws InisError (404) for an unknown or another
     * tenant's id. */
  ...
  constructor(opts: ClientOptions = {}) {
    ...
    this.sessions = {
      ...
      usage: (sessionId: string) => this._getSessionUsage(sessionId),
    ...
    this.volumes = {
      create: (opts?: { sizeGb?: number }) => this._createVolume(opts),
      list: () => this._listVolumes(),
      get: (volumeId: string) => this._getVolume(volumeId),
      resize: (volumeId: string, sizeGb: number) => this._resizeVolume(volumeId, sizeGb),
      delete: (volumeId: string) => this._deleteVolume(volumeId),
    };
  }
  ...
  private async _getSessionUsage(sessionId: string): Promise<SessionUsage> {
    const data = await request<Record<string, unknown>>(
      this.baseUrl, this.token, "GET", `/v1/sessions/${sessionId}/usage`, undefined, this.timeoutMs,
    );
    return mapSessionUsage(data);
  }
  ...
  private async _createVolume(opts?: { sizeGb?: number }): Promise<VolumeInfo> {
    const body = opts?.sizeGb !== undefined ? { size_gb: opts.sizeGb } : {};
    const data = await request<Record<string, unknown>>(
      this.baseUrl, this.token, "POST", "/v1/volumes", body, this.timeoutMs,
    );
    return mapVolume(data);
  }

  private async _listVolumes(): Promise<VolumeInfo[]> {
    const data = await request<{ volumes?: Record<string, unknown>[] }>(
      this.baseUrl, this.token, "GET", "/v1/volumes", undefined, this.timeoutMs,
    );
    return (data.volumes ?? []).map(mapVolume);
  }

  private async _getVolume(volumeId: string): Promise<VolumeInfo> {
    const data = await request<Record<string, unknown>>(
      this.baseUrl, this.token, "GET", `/v1/volumes/${encodeURIComponent(volumeId)}`, undefined, this.timeoutMs,
    );
    return mapVolume(data);
  }

  private async _resizeVolume(volumeId: string, sizeGb: number): Promise<VolumeInfo> {
    const data = await request<Record<string, unknown>>(
      this.baseUrl, this.token, "PATCH", `/v1/volumes/${encodeURIComponent(volumeId)}`,
      { size_gb: sizeGb }, this.timeoutMs,
    );
    return mapVolume(data);
  }

  private async _deleteVolume(volumeId: string): Promise<void> {
    await request<void>(
      this.baseUrl, this.token, "DELETE", `/v1/volumes/${encodeURIComponent(volumeId)}`, undefined, this.timeoutMs,
    );
  }
  ...
  /** Aggregate compute/storage usage and spend for the org.
   *
   * since/until (RFC3339) narrow the window; both default to the current
   * calendar month to now when omitted. groupBy: "label:<key>" additionally
   * breaks costToDateCents down by the value of each session's
   * labels[key] -- see UsageSummary.labels. */
  async usage(opts?: { since?: string; until?: string; groupBy?: string }): Promise<UsageSummary> {
    const params = new URLSearchParams();
    ...
    const query = params.toString() ? `?${params.toString()}` : "";
    const data = await request<Record<string, unknown>>(
      this.baseUrl, this.token, "GET", `/v1/usage${query}`, undefined, this.timeoutMs,
    );
