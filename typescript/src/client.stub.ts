...
export interface ConnectionAuth {
  type: "bearer" | "header";
  /** The upstream credential's plaintext value. Never echoed back. */
  secret: string;
  /** Required for "header" type; omitted (or "Authorization") for "bearer". */
  headerName?: string;
}
...
/**
 * A session-scoped credential lease binding one session to one external API
 * origin — the platform injects `authentication.secret` into requests that
 * clear `allow`, so sandbox code can call the origin directly without ever
 * holding the key. Not fully hidden from the sandbox's own traffic: an
 * origin that reflects request headers back can leak the value to code
 * running inside the session, so prefer a scoped, short-lived upstream
 * credential.
 */
export interface ConnectionCreate {
  /** Unique within the session. Lowercase alphanumerics, ".", "_", "-". */
  name: string;
  /** Exact https:// origin, hostname only. One Connection per origin. */
  origin: string;
  authentication: ConnectionAuth;
  allow: ConnectionAllow;
  /** Defaults to the session's maxLifetimeMs (or 1h); capped at 24h and
   * cannot exceed the session's own remaining lifetime. */
  ttlSeconds?: number;
}

/** Body for Session.rotateConnection() — ConnectionCreate minus `name`
 * (fixed by the URL). `origin` must match the existing Connection's origin. */
...
/** A Connection as reported on SessionInfo, redacted — never the secret.
 * "expired" is reported lazily, on the next read after the lease's TTL
 * lapses, not the instant it lapses. */
...
export interface SessionInfo {
   ...
   * paused or cold storage. Always present: true is the default, false
   * means the session opted out at create. */
  wakeOnHttp?: boolean;
  ...
  /** Connections bound to this session, redacted — never secret material. */
...
export interface CreateSessionOptions {
   ...
   * keeps today's behaviour: a request to an exposed port wakes a paused
  ...
  /** Inline Connections to create with this session (up to 16) — see
   * ConnectionCreate and SessionInfo.connections. */
...
export interface TemplateInfo {
  ...
  /** Set only on an alias entry, e.g. "python-3.13". */
  aliasOf?: string;
}
 ...
 * session or one-shot run mounts it. Deliberately node-agnostic — no node
 * id or placement hint anywhere in this shape. */
export interface VolumeInfo {
  ...
  sessionId?: string;
  /** When the durable copy last finished being written; `undefined` if the
   * volume has never completed a push. */
  lastSavedAt?: string;
}
...
export class Session {
   ...
   * rules/TTL. PUT /v1/sessions/{id}/connections/{name}. The Connection
   * must already exist and `origin` must match its current one — this does
   * not create a new Connection or move it to a different origin.
   *
   ...
   * of get()'s redacted `connections` field.
   */
  async rotateConnection(_name: string, _connection: ConnectionUpdate): Promise<ConnectionStatus> {
    throw new Error("not implemented");
  }

  /**
   * Revoke a Connection immediately: the stored credential is deleted and a
   * tombstone marks the origin revoked for the rest of the session's life.
   * Fails closed. DELETE /v1/sessions/{id}/connections/{name}.
   */
  async revokeConnection(_name: string): Promise<void> {
    throw new Error("not implemented");
  }
...
export class Client {
   ...
   * retired. */
  readonly volumes: {
    /** Create a new, empty persistent volume. Omitted sizeGb applies the
     * server default; over the maximum throws InisError (400). */
    ...
    /** Grow a volume's capacity (grow-only). Throws InisError (400) for a
     * same-size or smaller request; (409) while attached to a live or
     * paused session, or while an attach/detach is in progress. */
    resize(volumeId: string, sizeGb: number): Promise<VolumeInfo>;

    /** Permanently delete a persistent volume. Throws InisError (409)
  ...
  constructor(opts: ClientOptions = {}) {
    ...
    this.volumes = {
      create: (_opts?: { sizeGb?: number }): Promise<VolumeInfo> => {
        throw new Error("not implemented");
      },
      list: (): Promise<VolumeInfo[]> => {
        throw new Error("not implemented");
      },
      get: (_volumeId: string): Promise<VolumeInfo> => {
        throw new Error("not implemented");
      },
      resize: (_volumeId: string, _sizeGb: number): Promise<VolumeInfo> => {
        throw new Error("not implemented");
      },
      delete: (_volumeId: string): Promise<void> => {
        throw new Error("not implemented");
      },
    };
