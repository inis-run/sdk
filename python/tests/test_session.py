...
class TestConnections:
    def test_connection_spec_sent_on_create(self, fake_http):
        transport = fake_http(
            lambda method, path, params, body: json_response(
                200, {"session_id": "sess_new", "state": "live"}
            )
    ...
    def test_connection_spec_header_auth(self, fake_http):
        transport = fake_http(
            lambda method, path, params, body: json_response(
                200, {"session_id": "sess_new", "state": "live"}
            )
        ...
        session.__enter__()
        entry = transport.calls[0].json_body["connections"][0]
    ...
    def test_connections_accepts_plain_dicts(self, fake_http):
        transport = fake_http(
            lambda method, path, params, body: json_response(
                200, {"session_id": "sess_new", "state": "live"}
            )
        ...
        raw = {
            ...
            "authentication": {"type": "bearer", "secret": "sk_raw"},
            "allow": {"methods": ["GET"], "paths": ["/"]},
        }
        session = Session(base_url="https://api.inis.run", token="tok", connections=[raw])
        session.__enter__()
        assert transport.calls[0].json_body["connections"] == [raw]
    ...
    def test_omitted_when_unset(self, fake_http):
        ...
        Session(base_url="https://api.inis.run", token="tok").__enter__()
        assert "connections" not in transport.calls[0].json_body

    def test_get_exposes_redacted_connections_including_expired_state(self, fake_http):
        # "expired" is not hypothetical -- the server flips a Connection past
        # its expires_at to state="expired" on read (RedactedSessionConnections).
        fake_http(
            lambda method, path, params, body: json_response(
                200,
                {
        ...
        stripe = info.connections[0]
        assert stripe.name == "stripe"
        assert stripe.state == "active"
        assert stripe.header_name == "Authorization"
        assert stripe.allow.methods == ["GET"]
        assert stripe.allow.paths == ["/v1/*"]
        assert not hasattr(stripe, "secret")
        assert not hasattr(stripe, "authentication")
        assert info.connections[1].name == "acme"
        assert info.connections[1].state == "expired"

    def test_rotate_connection_sends_put_and_maps_response_without_secret(self, fake_http):
        transport = fake_http(
            lambda method, path, params, body: json_response(
                200,
                {
        ...
        status = make_session().rotate_connection(
            "stripe",
            origin="https://api.stripe.com",
    ...
    def test_revoke_connection_sends_delete(self, fake_http):
        transport = fake_http(lambda method, path, params, body: json_response(204, {}))
        make_session().revoke_connection("stripe")
        call = transport.calls[0]
        assert call.method == "DELETE"
        assert call.path == "https://api.inis.run/v1/sessions/sess_123/connections/stripe"
