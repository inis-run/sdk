...
class TestSessionsAPIList:
    ...
    async def test_list_passes_external_id_filter(self, fake_http_async):
        transport = fake_http_async(
            lambda method, path, params, body: json_response(
                200, {"sessions": [{"session_id": "sess_1", "state": "live"}]}
            )
        )
        await make_client().sessions.list(external_id="workflow-42")
...
class TestVolumesAPI:
    """Async twin of TestVolumesAPI in test_client.py -- same
    coverage (create with/without size, over-cap rejection, list
    empty/populated, clean not-found on get/delete, distinguishable
    conflict on delete-while-attached), exercised with await."""

    async def test_create_with_no_size_omits_size_gb(self, fake_http_async):
        transport = fake_http_async(
            lambda method, path, params, body: json_response(
                201,
        ...
        v = await make_client().volumes.create()
        assert transport.calls[0].json_body == {}
        assert v.id == "vol_1"

    async def test_create_with_explicit_size(self, fake_http_async):
        transport = fake_http_async(
            lambda method, path, params, body: json_response(
                201,
        ...
        v = await make_client().volumes.create(size_gb=5)
        assert transport.calls[0].json_body == {"size_gb": 5}
        assert v.size_gb == 5

    async def test_create_over_cap_raises_inis_error(self, fake_http_async):
        fake_http_async(
        ...
        with pytest.raises(InisError) as exc_info:
            await make_client().volumes.create(size_gb=999)
        assert exc_info.value.code == "bad_request"

    async def test_list_empty(self, fake_http_async):
        fake_http_async(lambda method, path, params, body: json_response(200, {"volumes": []}))
        assert await make_client().volumes.list() == []

    async def test_list_populated(self, fake_http_async):
        fake_http_async(
            lambda method, path, params, body: json_response(
                200,
                {
        ...
        volumes = await make_client().volumes.list()
        assert [v.id for v in volumes] == ["vol_1", "vol_2"]
        assert volumes[1].session_id == "ses_xyz"

    async def test_get_never_saved_has_no_last_saved_at(self, fake_http_async):
        """Never a zeroed/synthesized timestamp -- None is the signal that
        this volume has no durable copy yet."""
        fake_http_async(
            lambda method, path, params, body: json_response(
                200,
        ...
        v = await make_client().volumes.get("vol_1")
        assert v.last_saved_at is None

    async def test_get_saved_returns_last_saved_at(self, fake_http_async):
        fake_http_async(
            lambda method, path, params, body: json_response(
                200,
                {
    ...
    async def test_get_unknown_id_raises_clean_not_found(self, fake_http_async):
        fake_http_async(lambda method, path, params, body: error_response(404, "volume not found", code="not_found"))
        with pytest.raises(InisError) as exc_info:
            await make_client().volumes.get("vol_nonexistent")
        assert exc_info.value.code == "not_found"

    async def test_delete_unattached(self, fake_http_async):
        transport = fake_http_async(lambda method, path, params, body: json_response(204, {}))
        await make_client().volumes.delete("vol_1")
        assert transport.calls[0].method == "DELETE"
        assert transport.calls[0].path == "https://api.inis.run/v1/volumes/vol_1"

    async def test_delete_while_attached_raises_distinguishable_conflict(self, fake_http_async):
        fake_http_async(
            lambda method, path, params, body: error_response(
        ...
        with pytest.raises(InisError) as exc_info:
            await make_client().volumes.delete("vol_attached")
        assert exc_info.value.code == "conflict"

    async def test_resize_grows(self, fake_http_async):
        transport = fake_http_async(
            lambda method, path, params, body: json_response(
                200,
        ...
        v = await make_client().volumes.resize("vol_1", 5)
        call = transport.calls[0]
    ...
    async def test_resize_shrink_raises_bad_request(self, fake_http_async):
        fake_http_async(
            lambda method, path, params, body: error_response(
                400,
        ...
        with pytest.raises(InisError) as exc_info:
            await make_client().volumes.resize("vol_1", 3)
        assert exc_info.value.code == "bad_request"

    async def test_resize_while_attached_raises_distinguishable_conflict(self, fake_http_async):
        fake_http_async(
            lambda method, path, params, body: error_response(
        ...
        with pytest.raises(InisError) as exc_info:
            await make_client().volumes.resize("vol_attached", 5)
        assert exc_info.value.code == "conflict"
...
class TestSessionCreateOptions:
    ...
    async def test_external_id_forwarded_in_body_and_response(self, fake_http_async):
        transport = fake_http_async(
            lambda method, path, params, body: json_response(
                200, {"session_id": "sess_new", "state": "live", "external_id": "workflow-42"}
            )
        )
        session = await make_client().sessions.create(external_id="workflow-42")
        call = transport.calls[0]
        assert call.json_body["external_id"] == "workflow-42"
        assert session.session_id == "sess_new"

    async def test_idempotency_key_sent_as_header_not_body(self, fake_http_async):
        transport = fake_http_async(
            lambda method, path, params, body: json_response(
                200, {"session_id": "sess_new", "state": "live"}
            )
        )
        await make_client().sessions.create(idempotency_key="retry-key-1")
        call = transport.calls[0]
