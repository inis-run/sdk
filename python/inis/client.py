...
# (410, the session's home node is permanently gone, not merely unreachable),
# archive_unavailable
...
_RETRYABLE_CODES = frozenset(
    {
        ...
        "bad_gateway",
        # A concurrent duplicate request for the same Idempotency-Key is
        # still being processed -- retry and it resolves to the
        # same session once the other request finishes.
        ...
        "rate_limit_exceeded",
        "service_unavailable",
        # Account- and key-scoped concurrency caps -- same retryable-429
        # class as rate_limited/concurrency_limit_exceeded above, kept as
        # their own codes so a caller can tell which ceiling it hit. Both
        # are emitted with a Retry-After header.
...
class SessionInfo:
    ...
    session_id: str
    state: str
    """One of: creating, live, paused, lifetime_expired, failed, ended,
    interrupted, or node_lost. "lifetime_expired" is a read-time-only
    presentation of a paused session whose own max_lifetime_ms deadline has
    passed -- it's still just a paused session underneath (not destroyed),
    but resume() will always raise (410): check this before calling
    resume() on a long-paused session rather than discovering it from a
    failed resume. See status_reason."""
    ...
    didn't complete) or state=="lifetime_expired" (a short, user-safe reason
    resume() will refuse). Empty otherwise."""
    ...
    sessions may share one. See Sessions.create's external_id parameter."""
    last_active_at: str | None = None
    idle_timeout_ms: int | None = None
    idle_mode: str | None = None  # "ops" | "activity"
    """Which signal the idle timer trusts before pausing. "ops" (default) is
    op/PTY/keep_alive-based idle tracking; "activity" defers the pause while
    the guest shows CPU or network activity above threshold."""
    idle_busy_cpu_threshold_pct: int | None = None
    """Per-session override of the activity-mode CPU threshold. 0/None means
    this session uses the fleet-wide default."""
    idle_busy_network_bytes_per_sec: int | None = None
    """Per-session override of the activity-mode network threshold. 0/None
    means this session uses the fleet-wide default."""
    ...
    """Whether an inbound request to a preview URL may wake this session
    from paused or cold storage. True (the default) matches today's
    behaviour; False means a preview request must not wake it and returns
    404 instead, with no charge. Set at create via Sessions.create's
    wake_on_http parameter; explicit resume/exec calls are unaffected
    either way."""
    ...
    """The host-served self API's per-session gate state (SelfAPIStatus,
    e.g. {"events": False}) -- see /docs/self-api.md. A workload inside the
    VM reaches this surface directly at http://169.254.169.254; there is no
    dedicated SDK wrapper for it yet, so this is the raw dict rather than a
    typed object."""
    ...
    """Connections bound to this session, redacted — never any secret material."""
...
class UsageSummary:
    ...
    Client.usage(). Every figure is read from the same credit_transactions/
    usage_events ledger rows billing itself debited from -- never
    re-derived from duration x rate."""
    ...
    """Present only when group_by="label:<key>" was passed to usage(). Sums
    to cost_to_date_cents."""


@dataclass
class SessionUsage:
    """One session's lifetime usage/cost, see Session.usage() /
    Client.sessions.usage(). No window -- cost-to-date means since the
    session started."""
...
def _map_usage_summary(resp: dict[str, Any]) -> UsageSummary:
    labels: list[UsageLabelCost] | None = None
    if resp.get("labels") is not None:
        labels = [
            UsageLabelCost(label=item.get("label", ""), cost_cents=item.get("cost_cents", 0))
            for item in resp["labels"]
        ]
    return UsageSummary(
        period_start=resp.get("period_start"),
        since=resp.get("since"),
        until=resp.get("until"),
        executions=resp.get("executions", 0),
        vm_seconds=resp.get("vm_seconds", 0.0),
        vm_ms=resp.get("vm_ms", 0),
        paused_storage_cents=resp.get("paused_storage_cents", 0),
        volume_storage_cents=resp.get("volume_storage_cents", 0),
        cost_to_date_cents=resp.get("cost_to_date_cents", 0),
        pending_millicents=resp.get("pending_millicents", 0),
        compute_pending_millicents=resp.get("compute_pending_millicents", 0),
        paused_pending_millicents=resp.get("paused_pending_millicents", 0),
        volume_pending_millicents=resp.get("volume_pending_millicents", 0),
        balance_cents=resp.get("balance_cents", 0),
        labels=labels,
    )


def _map_session_usage(resp: dict[str, Any]) -> SessionUsage:
    return SessionUsage(
...
class TemplateInfo:
    ...
    # alias_of: set only on an alias entry — a documented bare name that
    # is not itself a published family but currently resolves to one, e.g.
    # name "python" with alias_of "python-3.13" and version "2026.07.10".
...
class ConnectionSpec:
    ...
    single external API origin (e.g. ``https://api.stripe.com``) and
    injects `secret` into outbound requests that match `methods`/`paths`.
    Used for the `connections` list on session create and for
    Session.rotate_connection.
    ...
    back out if the destination happens to reflect request headers (a
    debug/echo endpoint, an error body) -- we don't scrub response
    bodies before relaying them. Prefer a scoped, short-lived upstream
    credential (a Stripe restricted key, a GitHub fine-grained token, an
    AWS STS credential) so a value that does leak can't do more than
    `methods`/`paths` already allowed, and narrow `paths` away from any
    endpoint you know reflects requests.
    """
    ...
    """Defaults to this session's max_lifetime_ms, or 1 hour if the
    session has no lifetime cap. Capped at 24 hours regardless, and can
    never exceed the session's own remaining lifetime."""


def _connection_spec_payload(spec: ConnectionSpec | dict[str, Any]) -> dict[str, Any]:
    """Build one wire-format `connections[]` / rotate-body entry. Accepts
    a ConnectionSpec, or an already wire-shaped dict passed through as-is
    for callers who'd rather build the raw payload themselves."""
    if isinstance(spec, dict):
        return spec
    authentication: dict[str, Any] = {"type": spec.auth_type, "secret": spec.secret}
    if spec.header_name is not None:
        authentication["header_name"] = spec.header_name
    payload: dict[str, Any] = {
        "name": spec.name,
        "origin": spec.origin,
        "authentication": authentication,
        "allow": {"methods": spec.methods, "paths": spec.paths},
    }
    if spec.ttl_seconds is not None:
        payload["ttl_seconds"] = spec.ttl_seconds
    return payload
...
class ConnectionStatus:
    ...
    This redaction covers the inis.run API surface only: code running
    inside the session can still read the credential back out under the
    conditions described on ConnectionSpec.
    """

    name: str
    origin: str
    state: str  # "active" | "expired" | "revoked"
    """Not hypothetical: a Connection created with the default 1-hour TTL
    on a session that outlives it comes back state="expired" the next
    time you GET the session -- the server flips active rows past their
    expires_at on read."""
    header_name: str | None = None
...
def _map_connection_status(resp: dict[str, Any]) -> ConnectionStatus:
    allow_resp = resp.get("allow")
    allow = None
    if allow_resp is not None:
        allow = ConnectionAllow(
            methods=allow_resp.get("methods") or [],
            paths=allow_resp.get("paths") or [],
        )
...
class VolumeInfo:
    ...
    :meth:`_VolumesAPI.create` and referenced afterwards by its
    server-issued ``id`` on session-create/run's ``volume_id``. Naming an
    unrecognized or another tenant's id there raises :class:`InisError`
    (404) -- lazy creation is retired.
    ...
    ``None`` for a volume that has never completed a push -- never a zeroed
    or synthesized timestamp. A volume with no durable copy yet is
    genuinely distinct from one saved long ago; check for ``None`` rather
    than treating an empty/falsy value as "just now".
    """


def _map_volume(resp: dict[str, Any]) -> VolumeInfo:
    return VolumeInfo(
        id=resp["id"],
        size_gb=resp.get("size_gb", 0),
        created_at=resp.get("created_at", ""),
        attached=resp.get("attached", False),
        session_id=resp.get("session_id"),
        last_saved_at=resp.get("last_saved_at"),
    )
...
def _map_session_info(resp: dict[str, Any]) -> SessionInfo:
    ...
    raw_connections = resp.get("connections")
    connections = None
    if raw_connections is not None:
        connections = [_map_connection_status(c) for c in raw_connections]
    return SessionInfo(
        ...
        idle_mode=resp.get("idle_mode"),
        idle_busy_cpu_threshold_pct=resp.get("idle_busy_cpu_threshold_pct"),
        idle_busy_network_bytes_per_sec=resp.get("idle_busy_network_bytes_per_sec"),
...
class Session:
    ...
    def __enter__(self) -> "Session":
        ...
        if self._idle_mode:
            payload["idle_mode"] = self._idle_mode
        if self._name:
        ...
        resp = self._request("POST", "/v1/sessions", json=payload, headers=create_headers)
    ...
    def usage(self) -> SessionUsage:
        ...
        resp = self._request("GET", f"/v1/sessions/{self.session_id}/usage")
        return _map_session_usage(resp)
    ...
    def rotate_connection(
        ...
        *,
        ...
        `origin`/`secret`/`methods`/`paths`/`ttl_seconds` fully replace
        whatever Connection `name` currently has -- this is not a partial
        patch. Use it to rotate a leaked or expiring upstream credential,
        or to narrow/widen its allowlist, without tearing the session
        down. See ConnectionSpec for what `secret` protects against (and
        does not).
        """
        spec = ConnectionSpec(
            name=name,
    ...
    def revoke_connection(self, name: str) -> None:
        """Revoke Connection `name` immediately.

        This fails closed: a subsequent request to that origin gets a
        TLS-level failure, not a fallback to a direct, unauthenticated
        connection. Idempotent.
        """
        self._request("DELETE", f"/v1/sessions/{self.session_id}/connections/{name}")
...
class _SessionsAPI:
    ...
    def create(
        ...
        *,
        ...
        thread id) to stamp this session with, so it can be found later via
        list(external_id=...) without tracking session_id yourself. Not
        unique -- multiple sessions may share one.

        idempotency_key makes this create retry-safe: calling create() again
        with the same idempotency_key returns the ORIGINAL session instead
        of creating a second one or erroring on a name clash -- use it when
        a create's result may have been lost (e.g. a timeout) and you're
        not sure whether the session was actually created. Reusing a key
        with a DIFFERENT set of create arguments is an error (409 conflict),
        not a silent replay of the first call -- use a new key for a
        genuinely different create.
        ...
        connections creates one or more Connections inline with this
        session -- see ConnectionSpec. Unlike a connector, a Connection is
        scoped to a single origin plus an explicit method/path allowlist,
        has its own bounded TTL, and is created (or rotated/revoked) per
        session rather than registered once at the account level. Code in
        the session calls the bound origin's real hostname directly (no
        proxy path, no rewritten URL) and the platform injects the
        credential on the way out. A session can hold up to 16
        Connections, and one with any active Connection cannot be forked.
        To change or remove a Connection on a session already running,
        use Session.rotate_connection / Session.revoke_connection.
    ...
    def usage(self, session_id: str) -> SessionUsage:
        ...
        it."""
        resp = self._client._get(f"/v1/sessions/{session_id}/usage")
        return _map_session_usage(resp)
    ...
    def list(
        ...
        *,
        ...
        (e.g. every session a workflow spun up over its lifetime).
        """
        params: dict[str, Any] = {}
        if state:
            params["state"] = state
...
class _VolumesAPI:
    ...
    ``volume_id`` on session-create/run; an unrecognized or cross-tenant id
    there now raises :class:`InisError` (404) instead of silently
    provisioning one.
    """
    ...
    def create(self, *, size_gb: int | None = None) -> VolumeInfo:
        """Create a new, empty persistent volume and return its
        server-issued id. Omitted ``size_gb`` applies the server default;
        a value over the server's maximum raises :class:`InisError` (400)
        rather than being silently clamped down to it. Grow it later with
        :meth:`resize` (grow-only)."""
        payload: dict[str, Any] = {}
        if size_gb is not None:
            payload["size_gb"] = size_gb
        resp = self._client._post("/v1/volumes", payload)
        return _map_volume(resp)

    def list(self) -> list[VolumeInfo]:
        """List every persistent volume the caller owns."""
        resp = self._client._get("/v1/volumes")
    ...
    def get(self, volume_id: str) -> VolumeInfo:
        ...
        Raises :class:`InisError` (404) for an unknown or another tenant's
        id, indistinguishable from one that never existed."""
        resp = self._client._get(f"/v1/volumes/{volume_id}")
        return _map_volume(resp)

    def resize(self, volume_id: str, size_gb: int) -> VolumeInfo:
        """Grow a volume's capacity. Grow-only -- ``size_gb`` must be
        strictly greater than the volume's current size; raises
        :class:`InisError` (400, code "bad_request") for a same-size or
        smaller request rather than treating it as a no-op or a shrink, and
        for a value over the server's maximum. Raises :class:`InisError`
        (409, code "conflict") while the volume is attached to a live or
        paused session, or while an attach/detach is in progress -- detach
        it first. The larger size takes effect the next time the volume is
        attached, not immediately."""
        resp = self._client._patch(f"/v1/volumes/{volume_id}", {"size_gb": size_gb})
        return _map_volume(resp)

    def delete(self, volume_id: str) -> None:
        """Permanently delete a persistent volume (right-to-erasure/
        offboarding). Raises :class:`InisError` (409, code "conflict")
        while the volume is attached to a live or paused session --
        destroy that session first. Raises :class:`InisError` (404) for
        an unknown or another tenant's id."""
        self._client._delete(f"/v1/volumes/{volume_id}")
...
class Client:
    ...
    def session(
        ...
        *,
        ...
        idempotency_key makes this create retry-safe: calling session()/
        create() again with the same idempotency_key returns the ORIGINAL
        session instead of creating a second one or erroring on a name
        clash -- use it when a create's result may have been lost (e.g. a
        timeout) and you're not sure whether the session was actually
        created. Reusing a key with a DIFFERENT set of create arguments is
        an error (409 conflict), not a silent replay of the first call --
        use a new key for a genuinely different create.

        See Client.sessions.create for the full connectors/connections
    ...
    def usage(
        ...
        *,
        ...
        session's labels[key] -- see UsageSummary.labels.
        """
        params: dict[str, Any] = {}
        if since:
            params["since"] = since
        ...
        return _map_usage_summary(resp)
