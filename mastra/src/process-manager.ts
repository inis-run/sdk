...
export class InisProcessHandle extends ProcessHandle {
  ...
  async wait(): Promise<CommandResult> {
    ...
    // Verified live against the real API: a process that exits with code 0
    // currently comes back from `getProcess()` with NO `exit_code` field at
    // all (a wire-format bug in inis.run's own process-info structs, which
    // tag a plain `int` exit code with `omitempty` — so 0 and "never
    // recorded" are indistinguishable over the wire). Filed upstream as a
    // separate, non-Mastra-specific bug rather than worked around here:
    // there is no way for this adapter to tell "really exited 0" from
    // "exit code unknown" until the server stops dropping the zero value,
    // so `exitCode`/`success` below can be wrong (`-1`/`false`) for a
    // background process that actually succeeded.
    return {
