...
cancel/time-bound work — a generic retry helper plus scenarios against the
real, documented error contract.
...
- **Cancel / time-bound work** — `exec(..., timeout_ms=...)` and
  ...
  product gap, not a docs gap — a known limitation, not modelled as
  anything else here.
