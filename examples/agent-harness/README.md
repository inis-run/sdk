...
Five executable scenarios (#1237, epic #1229) organised around the jobs an
agent builder is actually trying to do, not around individual API
endpoints. Read [`../quickstart`](../quickstart) first — every example here
reuses its create/attach/destroy shape and its owned-vs-attached rule; this
directory is the follow-on set the quickstart itself points to.
...
- **Fork's cost/economics story** lives in
  [#912](https://github.com/4ver/inis.run/issues/912) and is linked to, not
  duplicated here.
...
- **Framework adapters** (Pydantic AI, Vercel AI SDK) are not used here on
  ...
  *pattern* every adapter is required to follow (#1229's "Definition of a
